/* Integrantes: Bernardo Kappaun de Oliveira
                Gabriel Borges Gambim
                Pedro Bezerra Iatauro */
...
void tutorial(){
    ...
    gotoxy(10, 17);
    printf("2. Jogadores podem coletar cartas da mesa ou roubar o monte dos adversarios;");
    gotoxy(10, 18);
...
void jogo(int nJogadores){
    ...
    // Definição das estruturas fundamentais
    Pilha* baralho;
    ...
    int cartasBaralho = TAMANHO_BARALHO; // Contador de cartas do baralho
    char indiceCarta;
    char opcao;
    ...
    // Cria os jogadores, sua mao e seu monte
    ...
    while(rodada->jogador->mao->prim != NULL){
        ...
        // Ao selecionar a carta, ela eh removida da mao
        cartaMao = removeCartaListaEnc2(rodada->jogador->mao, cartaSelecionada);
        int retorno = 0;
        while(retorno == 0){
            switch(opcao){
                case 'R':   // Roubar o Monte, se possível
                case 'r':
                    ...
                    break;
                case 'P':   // Pegar a mesa, se possível
                case 'p':
        ...
        // Quando acabam as cartas da mesa, pegam-se mais do baralho
        if(mesa->prim == NULL && !vaziaPilha(baralho)){
            mesa = criaMesa(baralho);
            cartasBaralho -= TAMANHO_MESA;
        }

        // Quando acabam as cartas da mao, pegam-se mais do baralho
