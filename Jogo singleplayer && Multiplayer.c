...
void jogo(int nJogadores){
    ...
    while(rodada->jogador->mao->prim != NULL){
        ...
        if(single_multi == 2) jogo_singleplayer(ordenacao, mesa); // Jogo singleplayer (o computador faz as jogadas)
...
void rodada(ListaCircEnc* ordem, ListaEnc2 *mesa, int chave_jogador){
    ...
    for(f = ordem->prim; f != NULL; f = f->prox){ // Muda jogador
        ...
        for(c = b->jogador.mao.prim; c != NULL; c = c->prox){ // Muda carta
            if(b->jogador.chave != f->jogador.chave){
                if(c->jogador.valor == f->jogador.monte.topo.jogador.valor){  
    ...
    while(c != NULL && umaJogada == 0){
        for(; d != NULL; d = d->prox){
            if(c->jogador.valor == d->jogador.valor){ // Se encontrar cartas de mesmo valor entre mão e mesa

                // Coloca na pilha do jogador
                empilhaPilhaEnc(b->jogador.monte, c->jogador);
                empilhaPilhaEnc(b->jogador.monte, d->jogador);

                // Remove da mesa e da mão
                removeInfoListaEnc2(b->jogador.mao, c->jogador.valor);
                removeInfoListaEnc2(mesa, d->jogador.valor);
                umaJogada = 1;
            }
        }
        c = c->prox;
    ...
    if(umaJogada == 0){
        ...
        removeInfoListaEnc2(b->jogador.mao, b->jogador.mao.prim.jogador.valor);
    }
}

void jogo_singleplayer(ListaCircEnc *ordem, ListaEnc2 *mesa){
